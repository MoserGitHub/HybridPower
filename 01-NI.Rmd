...
$$
where $z_{1-\alpha}=\Phi^{-1}(1-\alpha)$ with $\Phi^{-1}$, the quantile function of the standard Gaussian distribution function, which is equal to
$$
D\leq -z_{1-\alpha}\sqrt{\frac{n_0\sigma_1^2+n_1\sigma_0^2}{n_1n_0}}+\delta^*.
$$
Denote $D_{suc}:=-z_{1-\alpha}\sqrt{\frac{n_0\sigma_1^2+n_1\sigma_0^2}{n_1n_0}}+\delta^*$ as the risk difference for a 'success'. Then, for $\Phi$ being the probability distribution function of the standard Gaussian distribution,
$$
P_\delta(D\leq D_{suc})=\Phi\left(-z_{1-\alpha}-\sqrt{\frac{n_1n_0}{n_0\sigma_1^2+n_1\sigma_0^2}}(\delta-\delta^*)\right),
$$
...
$$
For an alternative point difference $\delta_A$, $P_{\delta_A}(D\leq D_{suc})$ is known as the frequentist power (or the **probability of rejection given $\delta_A$**). From that we have that
...
$\delta_A$ and $\delta^*$ are assumed as fixed and known constants in a frequentist approach. Their choices are of high importance, because all trial conclusions are based on those choices and affect the sample size calculation. The plot below shows how the sample size increase as $\delta^* \rightarrow \delta$.
...
Given the specified operating characteristics and parameters a sample size of 200 patients (100 per arm) is needed to reject the null hypothesis of inferiority.
...
Under an 'enthusiastic prior' we get an average power of 58%. This is substantially lower than the frequentist power of 80%.
