...
D\leq -z_{1-\alpha}\sqrt{\frac{n_0\sigma_1^2+n_1\sigma_0^2}{n_1n_0}}+\delta^*.
$$
By using a design prior for the treatment effect we want to take into account the uncertainty from the prior. The predictive distribution from a risk difference, say $\tilde D$, from future observation includes this uncertainty. The predictive distribution of $\tilde D$ is distributed as $\tilde D \sim N\left(d, \underbrace{\frac{\sigma_1^2}{n_1}+\frac{\sigma_0^2}{n_0}+\frac{\sigma_1^2}{m}+\frac{\sigma_0^2}{m}}_{\tilde\sigma^2}\right)$, see for example @Spiegelhalter2003. Let in the following $c_i=\frac{m+n_i}{m}$, then $\tilde\sigma^2=\frac{\sigma_1^2c_1}{n_1}+\frac{\sigma_0^2c_0}{n_0}$.
...
$$
Note that as $m \rightarrow \infty$, then $c_i \rightarrow 1$, $i\in {0,1}$, and $AP \rightarrow \Phi\left(-z_{1-\alpha}-\sqrt{\frac{n_1n_0}{n_0\sigma_1^2+n_1\sigma_0^2}}(d-\delta^*)\right)$, that is, the frequentist power at $d$.
