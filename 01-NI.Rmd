...
$$
$P_{\delta}(D\leq D_{suc}^{\delta^*})$ is the **'probability of rejection given $\delta$'**. Note that under a null value $\delta_0$, $P_{\delta_0}(D\leq D_{suc}^{\delta^*})$ is the 'type-I error' and under an alternative value $\delta_A$ is the 'frequentist power'. For an alternative value $\delta_A$ it holds that
...
Rufibach et al. give a closed a formula for the distribution of $RPR:=P_\Delta(D\leq D_{suc})$, where $\Delta \sim N(d,\tilde\sigma^2/m)$, and discuss the shape under different prior choices @Rufibach2016. In the following we use the wording 'random probability to reject` (RPR) similar to @Kunzmann2022.
...
$$
AP=\overbrace{P(D\leq D_{suc}, \Delta>\delta^*)}^{(1)}+\overbrace{P(D\leq D_{suc}, 0<\Delta\leq\delta^*)}^{(2)} \\
+\overbrace{P(D\leq D_{suc}, \Delta\leq0)}^{(3)}
$$
...
$AP \approx P(D\leq D_{suc}}, \Delta\leq0)$ (see @Spiegelhalter2003), pharma (favors AP, shortterm) vs regulators (favors the other, longterm), see discussion in Kunzmann et al.
