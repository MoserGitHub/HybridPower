...
Let $Y_{i,k} \sim^{iid} Bernoulli(p_i)$, $k=1,\dots,n_i$, $i\in \{0,1\}$. Let $\overline p_i=\frac{1}{n_i}\sum_{k\leq n_i} Y_{i,k}$, $i\in {0,1}$, and thus the risk difference $D=\overline p_1-\overline p_0$ is Gaussian distributed with $D \sim N\left(\delta, \frac{\sigma_1^2}{n_1}+\frac{\sigma_0^2}{n_0}\right)$, where $\sigma_i^2=p_i(1-p_i)$, $i\in \{0,1\}$. For notational purposes we denote $\sigma_{treat}^2=\frac{\sigma_1^2}{n_1}+\frac{\sigma_0^2}{n_0}$, that is, the variance for the 'treatment effect'.
...
$$
where $z_{1-\alpha}=\Phi^{-1}(1-\alpha)$. Simple algebra yields to
$$
...
$$
$P_{\delta}(D\leq D_{suc}^{\delta^*})$ is the **'probability of rejection given $\delta$'**. Note that under a null value $\delta=p1-p0$, $P_{\delta_0}(D\leq D_{suc}^{\delta^*})=\alpha$ is the 'type-I error' and under an alternative value $\delta_A$ is the 'frequentist power' $P_{\delta_A}(D\leq D_{suc}^{\delta^*})=1-\beta$. For an alternative value $\delta_A$ it holds that
...
We calculate the required sample size for the SAFE-SSPE trial. The following $R$ function can be used to calculate the required sample size with the following parameters:
...
D\leq -z_{1-\alpha}\sqrt{\frac{n_0\sigma_1^2+n_1\sigma_0^2}{n_1n_0}}+\delta^*.
$$
By using a design prior we take into account the uncertainty of the treatment effect. The predictive distribution from a risk difference from future observations, say $\tilde D\sim f(\tilde\delta)$, based on the prior information includes this uncertainty. For the Gaussian case, the predictive distribution of $\tilde D$ is distributed as $\tilde D \sim N\left(d, \underbrace{\frac{\sigma_1^2}{n_1}+\frac{\sigma_0^2}{n_0}+\frac{\sigma_1^2}{m}+\frac{\sigma_0^2}{m}}_{\tilde\sigma^2}\right)$, see for example @Spiegelhalter2003. Denote in the following $c_i=\frac{m+n_i}{m}$, then $\tilde\sigma^2=\frac{\sigma_1^2c_1}{n_1}+\frac{\sigma_0^2c_0}{n_0}$.

Suppose now that $D$ has a predictive distribution as described above, then
$$
\begin{aligned}
AP&=\int_{-\infty}^{-z_{1-\alpha}\sqrt{\frac{n_0\sigma_1^2+n_1\sigma_0^2}{n_1n_0}}+\delta^*}f(\tilde\delta)d\tilde\delta\\
&= \Phi\left(-z_{1-\alpha}\sqrt{\frac{n_0\sigma_1^2+n_1\sigma_0^2}{n_0\sigma_1^2c_1+n_1\sigma_0^2c_0}}-\sqrt{\frac{n_1n_0}{n_0\sigma_1^2c_1+n_1\sigma_0^2c_0}}(d-\delta^*)\right)\\
&=\Phi\left(\sqrt{\frac{n_0\sigma_1^2+n_1\sigma_0^2}{n_0\sigma_1^2c_1+n_1\sigma_0^2c_0}}\left[-z_{1-\alpha}-\sqrt{\frac{n_1n_0}{n_0\sigma_1^2+n_1\sigma_0^2}}(d-\delta^*)\right]\right),
\end{aligned}
$$
see for example @Grieve2022. Note that as $m \rightarrow \infty$, then $c_i \rightarrow 1$, $i\in {0,1}$, and $AP \rightarrow \Phi\left(-z_{1-\alpha}-\sqrt{\frac{n_1n_0}{n_0\sigma_1^2+n_1\sigma_0^2}}(d-\delta^*)\right)$, that is, the frequentist power at $d$.
...
Under an 'enthusiastic prior' we get an average power of 58%. For an 'skeptical prior' the average power decreases to 34%. These values are lower than the frequentist power of 80%.

---

Rufibach et al. give a closed a formula for the distribution of $RPR:=P_\Delta(D\leq D_{suc}^{\delta^*})$, where $\Delta \sim N(d,\tilde\sigma^2/m)$, and discuss the shape under different prior choices @Rufibach2016. In the following we use the wording 'random probability to reject` (RPR) similar to @Kunzmann2022.

For $0<y<1$, the random variable $RPR$ has a probability density function
$$
\begin{aligned}
f(y)&=\sqrt{\frac{m(n_0\sigma_1^2+n_1\sigma_0^2)}{n_1n_0(\sigma_1^2+\sigma_2^2)}}\left[ \phi\left(\Phi^{-1}(y) \right) \right]^{-1}\cdot\\& \quad \phi\left(-z_{1-\alpha}\sqrt{\frac{m(n_0\sigma_1^2+n_1\sigma_0^2)}{n_1n_0(\sigma_1^2+\sigma_2^2)}}-\sqrt{\frac{m}{\sigma_1^2+\sigma_2^2}}(d-\delta^*)-\sqrt{\frac{m(n_0\sigma_1^2+n_1\sigma_0^2)}{n_1n_0(\sigma_1^2+\sigma_2^2)}}\Phi^{-1}(y) \right),
\end{aligned}
$$
see @Rufibach2016.
...
For the case that $n_0=n_1=n$ and $\sigma_1^2=\sigma_0^2=\sigma^2$ the formula above reduces to

$$
\begin{aligned}
f(y)&=\sqrt{\frac{m}{n}}\left[ \phi\left(\Phi^{-1}(y) \right) \right]^{-1}\phi\left(\sqrt{\frac{m}{n}}\left[-z_{1-\alpha}-\sqrt{\frac{n}{2\sigma^2}}(d-\delta^*)-\Phi^{-1}(y)\right] \right), \quad 0<y<1.
\end{aligned}
$$
...
The cumulative distribution function of $RPR$ is

$$
\begin{aligned}
P(RPR\leq y)=1-\Phi&\left(-z_{1-\alpha}\sqrt{\frac{m(n_0\sigma_1^2+n_1\sigma_0^2)}{n_1n_0(\sigma_1^2+\sigma_2^2)}}-\sqrt{\frac{m}{\sigma_1^2+\sigma_2^2}}(d-\delta^*)-\sqrt{\frac{m(n_0\sigma_1^2+n_1\sigma_0^2)}{n_1n_0(\sigma_1^2+\sigma_2^2)}}\Phi^{-1}(y) \right), \quad 0<y<1.
\end{aligned}
$$
...
$$
\begin{aligned}
P(RPR\leq y)=1-\Phi&\left(\sqrt{\frac{m}{n}}\left[-z_{1-\alpha}-\sqrt{\frac{n}{2\sigma^2}}(d-\delta^*)-\Phi^{-1}(y)\right] \right), \quad 0<y<1.
\end{aligned}
$$

---

This allows the calculation of probability statements as $P(RPR>\gamma)$:
...
data_rpr$p_rpr <- pnorm(-sqrt(data_rpr$m_range*data_rpr$n_0_range*sd_1^2+data_rpr$m_range*data_rpr$n_1_range*sd_0^2)/(sqrt(data_rpr$n_1_range*data_rpr$n_0_range*(sd_0^2+sd_1^2)))*qnorm(1-alpha)-sqrt(data_rpr$m_range)/sqrt((sd_0^2+sd_1^2))*(prior_mean-delta_star)-sqrt(data_rpr$m_range*data_rpr$n_0_range*sd_1^2+data_rpr$m_range*data_rpr$n_1_range*sd_0^2)/(sqrt(data_rpr$n_1_range*data_rpr$n_0_range*(sd_0^2+sd_1^2)))*qnorm(data_rpr$gamma))

ggplot(data_rpr, aes(x=gamma, y=p_rpr, colour=factor(m_range)))+geom_line(linewidth=1)+theme_bw()+theme(panel.grid.minor = element_blank())+scale_color_brewer("Prior sample size", palette="Reds")+xlab("Threshold gamma")+ylab("Probability that RPR is greater than threshold gamma")+scale_y_continuous(breaks=seq(0,1,0.1))+scale_x_continuous(breaks=seq(0,1,0.2))+labs(caption=str_glue("Calculated based on planned sample size of ", round(n_0_range,0), " for n0."))
```

For a specific threshold $\gamma=0.6$ we obtain
...
data_rpr_skep$p_rpr <- pnorm(-sqrt(data_rpr_skep$m_range*data_rpr_skep$n_0_range*sd_1^2+data_rpr_skep$m_range*data_rpr_skep$n_1_range*sd_0^2)/(sqrt(data_rpr_skep$n_1_range*data_rpr_skep$n_0_range*(sd_0^2+sd_1^2)))*qnorm(1-alpha)-sqrt(data_rpr_skep$m_range)/sqrt((sd_0^2+sd_1^2))*(prior_mean-delta_star)-sqrt(data_rpr_skep$m_range*data_rpr_skep$n_0_range*sd_1^2+data_rpr_skep$m_range*data_rpr_skep$n_1_range*sd_0^2)/(sqrt(data_rpr_skep$n_1_range*data_rpr_skep$n_0_range*(sd_0^2+sd_1^2)))*qnorm(gamma))

data_rpr_skep$type <- "Skeptical"

data_rpr <- rbind(data_rpr, data_rpr_skep)

ggplot(data_rpr, aes(x=n_1_range, y=p_rpr, colour=factor(m_range), linetype=type))+geom_line(linewidth=1)+theme_bw()+theme(panel.grid.minor = element_blank())+scale_color_brewer("Prior sample size", palette="Reds")+xlab("Planned sample size active group")+ylab(str_glue("Probability that RPR is greater than ", gamma*100, "%"))+scale_x_continuous(breaks=c(0, 50, 100, 200, 300, 400, 500))+geom_vline(xintercept=100, linetype="dashed")+scale_y_continuous(breaks=seq(0,1,0.1))+scale_linetype("Prior type")
```

AP integrates over whole $\delta$ range, including 'non-favorable' regions.
$$
AP=\overbrace{P(D\leq D_{suc}^{\delta^*}, \Delta>\delta^*)}^{(1)}+\overbrace{P(D\leq D_{suc}^{\delta^*}, 0<\Delta\leq\delta^*)}^{(2)} \\
+\overbrace{P(D\leq D_{suc}^{\delta^*}, \Delta\leq0)}^{(3)}
$$
...
$AP \approx P(D\leq D_{suc}^{\delta^*}, \Delta\leq0)$ (see @Spiegelhalter2003), pharma (favors AP, shortterm) vs regulators (favors the other, longterm), see discussion in Kunzmann et al.
...
A proper Bayesian approach uses the posterior distribution to define 'Bayesian significance': $S^B:=P(\Delta\leq\delta^*|data)=1-\epsilon$. Remember $D=p_1-p_0$ with $D \sim N(\delta, \frac{\sigma_1^2}{n_1}+\frac{\sigma_0^2}{n_0})$. Suppose that $\Delta \sim N(d, \tilde\sigma^2/m)$ then the posterior distribution given $D$ is distributed as
$$
\Delta|D \sim N\left(\frac{\frac{n_1n_0}{n_0\sigma_1^2+n_1\sigma_0^2}\delta+\frac{m}{\sigma_1^2+\sigma_0^2}D}{\frac{n_1n_0}{n_0\sigma_1^2+n_1\sigma_0^2}+\frac{m}{\sigma_1^2+\sigma_0^2}}, \frac{1}{\frac{n_1n_0}{n_0\sigma_1^2+n_1\sigma_0^2}+\frac{m}{\sigma_1^2+\sigma_0^2}}\right)
$$
...
$$
\Delta|D \sim N\left(\frac{n\delta+mD}{n+m}, \frac{2\sigma^2}{n+m} \right).
$$

---

Bases on the above specified 'Bayesian significance' we are interested whether the upper $(1-\epsilon)$ credible interval is smaller than the non-inferiority margin, that is

$$
\frac{\frac{n_1n_0}{n_0\sigma_1^2+n_1\sigma_0^2}D+\frac{m}{\sigma_1^2+\sigma_0^2}d}{\frac{n_1n_0}{n_0\sigma_1^2+n_1\sigma_0^2}+\frac{m}{\sigma_1^2+\sigma_0^2}}+z_{1-\epsilon}\frac{1}{\sqrt{\frac{n_1n_0}{n_0\sigma_1^2+n_1\sigma_0^2}+\frac{m}{\sigma_1^2+\sigma_0^2}}}\leq \delta^*.
$$
Simple algebra yields to

$$
\begin{aligned}
D\leq -z_{1-\epsilon}&\sqrt{\frac{n_1n_0}{n_0\sigma_1^2+n_1\sigma_0^2}+\frac{m}{\sigma_1^2+\sigma_0^2}}\frac{n_0\sigma_1^2+n_1\sigma_0^2}{n_1n_0}+ \\&\delta^*\left(1+\frac{m(n_0\sigma_1^2+n_1\sigma_0^2)}{n_1n_0(\sigma_1^2+\sigma_0^2)}\right)-\frac{m(n_0\sigma_1^2+n_1\sigma_0^2)}{n_1n_0(\sigma_1^2+\sigma_0^2)}d
\end{aligned}
$$
Thus
$$
\begin{aligned}
P(S^B|\delta)&=\Phi\left(-z_{1-\epsilon}\left\{\sqrt{1+\frac{m(n_0\sigma_1^2+n_1\sigma_0^2)}{n_1n_0(\sigma_1^2+\sigma_0^2)}}\right\}- \right. \\
&\left. \frac{m\sqrt{n_0\sigma_1^2+n_1\sigma_0^2}}{\sqrt{n_1n_0}(\sigma_1^2+\sigma_0^2)}d-\sqrt{\frac{n_1n_0}{n_0\sigma_1^2+n_1\sigma_0^2}}\left\{\delta-\delta^*\left[1+\frac{m(n_0\sigma_1^2+n_1\sigma_0^2)}{n_1n_0(\sigma_1^2+\sigma_0^2)} \right]\right\} \right)
\end{aligned}
$$
