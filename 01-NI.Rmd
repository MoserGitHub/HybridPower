...
We calculate the AP under the assumed prior distributions, operating characteristics and parameters for the SAFE-SSPE trial.
...
Under an 'enthusiastic prior' we get an average power of 58%. This is substantially lower than the frequentist power of 80% (which holds under the alternative hypothesis).
...
From the above formula one can derive probability statements as $P(RPR>\gamma)$, say $\gamma=0.8$, from

$$
\begin{aligned}
P(RPR>\gamma)&=\Phi\left(-z_{1-\alpha}\frac{\sqrt{mn_0\sigma_1^2+mn_1\sigma_0^2}}{\sqrt{n_1n_0(\sigma_1^2+\sigma_2^2)}}-\sqrt{\frac{m}{\sigma_1^2+\sigma_2^2}}(d-\delta^*)-\frac{\sqrt{mn_0\sigma_1^2+mn_1\sigma_0^2}}{\sqrt{n_1n_0(\sigma_1^2+\sigma_2^2)}}\Phi^{-1}(y) \right),
\end{aligned}
$$
...
ggplot(data_rpr, aes(x=n_0_range, y=p_rpr, colour=factor(m_range), linetype=type))+geom_line(linewidth=1)+theme_bw()+theme(panel.grid = element_blank())+scale_color_brewer("Prior sample size", palette="Reds")+xlab("Final sample size (one arm)")+ylab("P(RPR>0.8)")+scale_x_continuous(breaks=c(0, 100, 250, 500, 750, 1000))+geom_vline(xintercept=100, linetype="dashed")+scale_y_continuous(breaks=c(0,0.5,0.8,1))+scale_linetype("Prior type")
```
...
A proper Bayesian approach ises posterior distribution to define 'Bayesian significance': $S^B:=P(\Delta\leq\delta^*|data)=1-\epsilon$, (not $\alpha$)
Remember $D=p_1-p_0$ with $D \sim N(\delta, \frac{\sigma_1^2}{n_1}+\frac{\sigma_0^2}{n_0})$. Suppose that $\Delta \sim N(d, \tilde\sigma^2/n_0)$ then the posterior distribution given $D$ is distributed as
$$
...
$$
For the case that $n_0=n_1=n$ and $\sigma_1^2=\sigma_0^2=\sigma^2$ this reduces to
...
ggplot(data_power, aes(x=delta, y=y, colour=type))+geom_line()+theme_bw()+theme(panel.grid.minor = element_blank())+scale_colour_brewer("", palette="Dark2")+ylab("Probability of rejection")+ylab("Probability to reject")+scale_y_continuous(breaks=c(0,0.5,0.8,1))+ggtitle(paste0(paste0("n=", round(n_0+n_1,0)), " (total sample size)"))+geom_vline(xintercept=0, linetype="dashed")+geom_vline(xintercept=0.035, linetype="dashed")
